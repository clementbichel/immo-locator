(() => {
  ...
  function calculateMatchScore(adData, ademeItem) {
    ...
    const dateItem = ademeItem.date_etablissement_dpe ? new Date(ademeItem.date_etablissement_dpe) : null;
    if (dateAd && dateItem && !isNaN(dateItem.getTime())) {
      const diffTime = Math.abs(dateAd.getTime() - dateItem.getTime());
      const diffDays = Math.ceil(diffTime / (1e3 * 60 * 60 * 24));
      score -= diffDays * 2;
    }
    if (adData.conso_prim && adData.conso_prim !== "Non trouv\xE9" && ademeItem.conso_5_usages_par_m2_ep) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    async function extractRealEstateData() {
      ...
      if (!window.location.href.includes("/ventes_immobilieres/") && !window.location.href.includes("/locations/")) {
        return { error: "Cette extension ne fonctionne que pour les ventes immobili\xE8res et les locations." };
      }
      const data = {
          ...
          if (ad) {
            ...
            if (ad.attributes) {
              ...
              const gesAttr = attributes.find(
                (a) => a.key === "ges_rate" || a.label && (a.label.toLowerCase() === "ges" || a.label.toLowerCase().includes("gaz \xE0 effet de serre"))
              );
              if (gesAttr) data.ges = (gesAttr.value_label || gesAttr.value).toUpperCase();
              const dateAttr = attributes.find((a) => a.label && a.label.includes("Date de r\xE9alisation"));
      ...
      if (data.dpe === "Non trouv\xE9") {
        const dpeBadge = document.querySelector('[aria-label*="Diagnostic \xE9nerg\xE9tique"], [aria-label*="Classe \xE9nergie"]');
        if (dpeBadge) {
      ...
      if (data.ges === "Non trouv\xE9") {
        const gesBadge = document.querySelector('[aria-label*="Indice \xE9mission de gaz \xE0 effet de serre"], [aria-label*="GES"]');
        if (gesBadge) {
      ...
      function findValueFromVisualScale(labelPart) {
        ...
        const labelEl = allElements.find((el) => el.innerText.includes(labelPart) && el.innerText.length < 50);
        ...
        let active = findOutlier2(metrics, "height");
        if (!active) active = findOutlier2(metrics, "width");
        if (!active) active = findOutlier2(metrics, "fontSize");
        if (!active) active = findOutlier2(metrics, "fontWeight");
        if (active) {
    ...
    async function executeAdemeSearch(data) {
      ...
      try {
        ...
        if (result.results && result.results.length > 0) {
          ...
          let html = '<p><strong>Correspondances trouv\xE9es :</strong></p><ul style="padding-left: 20px; margin-top: 5px;">';
          sortedResults.forEach((item) => {
            ...
            <a href="${mapsLink}" target="_blank" style="display: inline-block; margin-top: 4px; font-size: 0.85em; color: #3498db; text-decoration: none;">\u{1F4CD} Voir sur Google Maps</a>
          </li>`;
          });
          html += "</ul>";
          ademeResults.innerHTML = html;
        } else {
          ademeResults.innerHTML = "<p>Aucun DPE correspondant trouv\xE9 avec ces crit\xE8res stricts.</p>";
        }
        if (!data.zipcode || data.zipcode === "Non trouv\xE9") {
          ademeResults.innerHTML = '<p style="color: orange;">Code postal non trouv\xE9, impossible de chercher dans la base.</p>';
          return;
        }
        if (!data.date_diag || data.date_diag === "Non trouv\xE9") {
          ademeResults.innerHTML = '<p style="color: orange;">Date du diagnostic non trouv\xE9e, recherche annul\xE9e.</p>';
          return;
        }
      } catch (error) {
