...
const numericString = z.preprocess(
  (val) => {
    if (val === null || val === undefined || val === 'Non trouvé') return undefined;
...
router.post('/', (req, res) => {
  ...
  if (!extractedResult.success) {
    logger.warn(
      { issues: extractedResult.error.issues, extracted },
      'Report rejected: invalid extracted'
    );
    return res
      .status(400)
      .json({ error: 'INVALID_EXTRACTED', message: 'Données extraites invalides.' });
  }
