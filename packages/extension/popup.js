...
(() => {
  ...
  function calculateMatchScore(adData, ademeItem) {
    let totalWeight = 0;
    let weightedSum = 0;
    if (adData.surface !== null && adData.surface !== void 0 && ademeItem.surface_habitable_logement !== null && ademeItem.surface_habitable_logement !== void 0) {
      const { weight, maxDeviation } = MATCH_CONFIG.surface;
      totalWeight += weight;
      weightedSum += weight * percentFieldMatch(ademeItem.surface_habitable_logement, adData.surface, maxDeviation);
    }
    ...
    if (adData.conso_fin !== null && adData.conso_fin !== void 0 && ademeItem.conso_5_usages_par_m2_ef !== null && ademeItem.conso_5_usages_par_m2_ef !== void 0) {
      const { weight, maxDeviation } = MATCH_CONFIG.conso_fin;
      totalWeight += weight;
      weightedSum += weight * percentFieldMatch(ademeItem.conso_5_usages_par_m2_ef, adData.conso_fin, maxDeviation);
    }
    if (adData.conso_prim !== null && adData.conso_prim !== void 0 && ademeItem.conso_5_usages_par_m2_ep !== null && ademeItem.conso_5_usages_par_m2_ep !== void 0) {
      const { weight, maxDeviation } = MATCH_CONFIG.conso_prim;
      totalWeight += weight;
      weightedSum += weight * percentFieldMatch(ademeItem.conso_5_usages_par_m2_ep, adData.conso_prim, maxDeviation);
    }
    if (totalWeight === 0) return 0;
    return Math.round(weightedSum / totalWeight * 100);
  }
  function processResults(adData, ademeResults) {
    const scored = ademeResults.map((item) => ({
  ...
  var ERROR_MESSAGES = {
    // Network errors
    [ERROR_CODES.NETWORK_TIMEOUT]: "La connexion a expir\xE9. V\xE9rifiez votre connexion internet et r\xE9essayez.",
    ...
    // Data extraction errors
    [ERROR_CODES.INVALID_PAGE]: "Cette extension ne fonctionne que pour les ventes immobili\xE8res et les locations.",
  ...
  function validateSearchData(data) {
    const missing = [];
    const warnings = [];
    const hasLocation = data.zipcode && data.zipcode !== "Non trouv\xE9" || data.city && data.city !== "Non trouv\xE9";
  ...
  async function fetchAdeme(payload) {
    ...
    try {
      ...
      err.code = (e == null ? void 0 : e.name) === "TimeoutError" ? ERROR_CODES.NETWORK_TIMEOUT : ERROR_CODES.NETWORK_ERROR;
      throw err;
    }
    if (!response.ok) {
  ...
  function createLocationResultsList(results, getMapsLink, getScoreColor2, broadened = false) {
    ...
    const confident = !broadened && ((_a = results[0]) == null ? void 0 : _a.score) >= CONFIDENT_SCORE;
    const title = createElement("p");
    const strong = createElement("strong", confident ? "Adresses trouv\xE9es :" : "Pistes possibles :");
    ...
    if (!confident) {
      container.appendChild(
        createElement(
          "p",
          broadened ? "Recherche \xE9largie : le GES et les consommations ont \xE9t\xE9 ignor\xE9s faute de correspondance exacte. V\xE9rifiez la surface et la date du diagnostic." : "Correspondance incertaine : v\xE9rifiez la surface et la date du diagnostic avant de vous fier \xE0 ces adresses.",
  ...
  function isValidSelogerRealEstateUrl(url) {
    ...
    if (!parsed) return false;
    return parsed.pathname.startsWith("/annonces/achat/") || parsed.pathname.startsWith("/annonces/locations/") || SELOGER_DETAIL_RE.test(parsed.pathname);
  }
  ...
  var TRIGGER_AT = 6;
  var STORE_URLS = {
    chrome: "https://chromewebstore.google.com/detail/immo-locator/okglkdgbdbnikojffmjpodmakgjmlpda/reviews",
    firefox: "https://addons.mozilla.org/fr/firefox/addon/immo-locator/reviews/"
  };
  function getStoreReviewUrl() {
    var _a;
    const browserApi = globalThis.browser || globalThis.chrome;
    const isFirefox = (_a = browserApi == null ? void 0 : browserApi.runtime) == null ? void 0 : _a.getURL("").startsWith("moz-extension://");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    async function extractRealEstateData() {
      ...
      if (!window.location.href.includes("/ventes_immobilieres/") && !window.location.href.includes("/locations/")) {
        return {
          error: "Cette extension ne fonctionne que pour les ventes immobili\xE8res et les locations."
        };
      }
      const data = {
      ...
      try {
        ...
        if (nextDataScript) {
          ...
          const pageProps = (_a = jsonData == null ? void 0 : jsonData.props) == null ? void 0 : _a.pageProps;
          ...
          if (ad) {
            ...
            if (ad.attributes) {
              ...
              if (surfaceAttr) data.surface = surfaceAttr.value_label || surfaceAttr.value + " m\xB2";
              const terrainAttr = findAttr("land_plot_surface", "terrain");
              if (terrainAttr) data.terrain = terrainAttr.value_label || terrainAttr.value + " m\xB2";
      ...
      if (data.zipcode === "Non trouv\xE9" || data.city === "Non trouv\xE9") {
        try {
          const routerComponents = (_c = (_b = window.next) == null ? void 0 : _b.router) == null ? void 0 : _c.components;
          if (routerComponents) {
            for (const comp of Object.values(routerComponents)) {
              const location = (_f = (_e = (_d = comp == null ? void 0 : comp.props) == null ? void 0 : _d.pageProps) == null ? void 0 : _e.ad) == null ? void 0 : _f.location;
              if (location) {
      ...
      function findValueFromVisualScale(labelPart) {
        ...
        while (container && attempts < 4) {
          ...
          letters = candidates.filter((el) => {
            return /^[A-G]$/.test(el.innerText.trim()) && labelEl.compareDocumentPosition(el) & Node.DOCUMENT_POSITION_FOLLOWING;
          });
          if (letters.length >= 5) {
            if (letters.length > 7) {
      ...
      if (data.dpe === "Non trouv\xE9") {
        const visualDpe = findValueFromVisualScale("Classe \xE9nergie") || findValueFromVisualScale("Diagnostic \xE9nerg\xE9tique");
        if (visualDpe) data.dpe = visualDpe;
      }
      if (data.ges === "Non trouv\xE9") {
        const visualGes = findValueFromVisualScale("GES") || findValueFromVisualScale("Gaz \xE0 effet de serre");
    ...
    function extractSelogerData() {
      ...
      const isSelogerAd = path.startsWith("/annonces/achat/") || path.startsWith("/annonces/locations/") || /^\/\d+\/detail\.htm$/.test(path);
      if (!isSelogerAd) {
        return {
          error: "Cette extension ne fonctionne que sur les pages d\u2019annonces SeLoger (achat ou location)."
        };
      }
      const data = {
        surface: "Non trouv\xE9",
      ...
      try {
        ...
        const classified = (_b = (_a = state == null ? void 0 : state.app_cldp) == null ? void 0 : _a.data) == null ? void 0 : _b.classified;
        if (!classified) {
        ...
        const location = ((_c = sections.location) == null ? void 0 : _c.address) || sections.location;
        if (location) {
        ...
        if (Array.isArray(facts)) {
          const living = facts.find((f) => /^(livingSpace|surface)$/i.test((f == null ? void 0 : f.type) || ""));
          const plot = facts.find((f) => (f == null ? void 0 : f.type) === "plotSpace");
          if (living == null ? void 0 : living.splitValue) data.surface = living.splitValue + " m\xB2";
          if (plot == null ? void 0 : plot.splitValue) data.terrain = plot.splitValue + " m\xB2";
        } else {
          const space = (_g = (_f = (_e = classified.legacyTracking) == null ? void 0 : _e.products) == null ? void 0 : _f[0]) == null ? void 0 : _g.space;
          if (typeof space === "number") data.surface = space + " m\xB2";
        }
        const scales = (_j = (_i = (_h = sections.energy) == null ? void 0 : _h.certificates) == null ? void 0 : _i[0]) == null ? void 0 : _j.scales;
        if (Array.isArray(scales)) {
          const energyScale = scales.find((s) => /^FR_ENERGY/.test((s == null ? void 0 : s.type) || ""));
          const ghgScale = scales.find((s) => /^FR_GHG/.test((s == null ? void 0 : s.type) || ""));
          if ((_k = energyScale == null ? void 0 : energyScale.efficiencyClass) == null ? void 0 : _k.rating) {
            data.dpe = String(energyScale.efficiencyClass.rating).toUpperCase();
          }
          if ((_l = ghgScale == null ? void 0 : ghgScale.efficiencyClass) == null ? void 0 : _l.rating) {
            data.ges = String(ghgScale.efficiencyClass.rating).toUpperCase();
          }
          if (Array.isArray(energyScale == null ? void 0 : energyScale.values)) {
            const consoEntry = energyScale.values.find((v) => /consommation/i.test((v == null ? void 0 : v.label) || ""));
    ...
    async function executeLocationSearch(data) {
      ...
      try {
        ...
        searchBtn.disabled = false;
        clearElement(locationResults);
        const errorMessage = error.code && ERROR_CODES[error.code] ? getErrorMessage(error.code) : error.message || "Erreur lors de la recherche.";
