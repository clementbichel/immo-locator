...
rsync -avz -e "ssh -p ${VPS_PORT}" --exclude 'node_modules' --exclude '.env' --exclude '.git' \
  ~/immo-locator-api/ "${VPS_HOST}:${APP_DIR}/"

echo "2. Installing dependencies on VPS..."
ssh -p "${VPS_PORT}" "${VPS_HOST}" "cd ${APP_DIR} && npm install --production"

echo "3. Restarting app with PM2..."
ssh -p "${VPS_PORT}" "${VPS_HOST}" "cd ${APP_DIR} && pm2 restart immo-locator-api 2>/dev/null || pm2 start src/index.js --name immo-locator-api && pm2 save"

echo "4. Checking health..."
sleep 2
ssh -p "${VPS_PORT}" "${VPS_HOST}" "curl -s http://127.0.0.1:3000/health"
echo ""
