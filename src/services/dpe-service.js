...
export function calculateMatchScore(adData, ademeItem) {
  ...
  if (adData.surface != null && ademeItem.surface_habitable_logement != null) {
    const diffPercent = (Math.abs(adData.surface - ademeItem.surface_habitable_logement) / adData.surface) * 100;
    score -= diffPercent * 2;
  }
  ...
  if (dateAd && dateItem) {
    const diffDays = Math.round(Math.abs(dateAd.getTime() - dateItem.getTime()) / (1000 * 60 * 60 * 24));
    score -= diffDays * 2;
  }

  if (adData.conso_prim != null && ademeItem.conso_5_usages_par_m2_ep != null) {
    const diffPercent = (Math.abs(adData.conso_prim - ademeItem.conso_5_usages_par_m2_ep) / adData.conso_prim) * 100;
    score -= diffPercent;
  }
