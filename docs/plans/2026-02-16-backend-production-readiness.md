...
## Task 1 : Validation des env vars au démarrage
...
## Task 2 : Middleware d'erreur global
...
## Task 3 : Timeout sur les appels ADEME
...
## Task 4 : Validation d'entrée avec Zod
...
## Task 9 (nice to have) : Mettre à jour le script de déploiement
...
| Ordre | Task                          | Effort | Impact                   |
| ----- | ----------------------------- | ------ | ------------------------ |
| 1     | Task 1 — Validation env vars  | 5 min  | Fail-fast au deploy      |
| 2     | Task 2 — Error handler global | 10 min | Pas de 500 HTML          |
| 3     | Task 3 — Timeout ADEME        | 5 min  | Pas de requêtes bloquées |
| 4     | Task 4 — Validation Zod       | 30 min | Sécurité des entrées     |
| 5     | Task 5 — CORS vrais origins   | 5 min  | Restreindre l'accès      |
| 6     | Task 6 — Logging Pino         | 30 min | Debugging en prod        |
| 7     | Task 7 — UptimeRobot          | 10 min | Alertes si down          |
| 8     | Task 8 — Cache LRU            | 20 min | Performance              |
| 9     | Task 9 — Deploy script        | 5 min  | Cleanup                  |
...
- [ ] Le serveur refuse de démarrer sans `ADEME_API_URL`
- [ ] Une requête avec un body invalide renvoie une 400 JSON propre
- [ ] Une requête quand l'API ADEME est down renvoie une 502 en < 11s
- [ ] Les logs sont en JSON structuré (pas de `console.log`)
- [ ] CORS bloque les requêtes depuis un origin non autorisé
- [ ] UptimeRobot envoie une alerte test
